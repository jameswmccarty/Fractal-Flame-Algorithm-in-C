...
void coeff_init(flame *fractal) {
	...
	for(i=0; i < fractal->n; i++)
	{
		...
		fractal->coarray[i].r = fractal->R != -1 ? (unsigned char) fractal->R : (unsigned char) (64 + RANDR(64,256));
		fractal->coarray[i].g = fractal->G != -1 ? (unsigned char) fractal->G : (unsigned char) (64 + RANDR(64,256));
		fractal->coarray[i].b = fractal->B != -1 ? (unsigned char) fractal->B : (unsigned char) (64 + RANDR(64,256));
	}	

	if(fractal->pallete != NULL) {
		i=0;	
		while((fscanf(fractal->pallete, "%d %d %d\n", &file_r, &file_g, &file_b) != EOF)
				&& i < fractal->n) {

			fractal->coarray[i].r = (unsigned char) file_r;
			fractal->coarray[i].g = (unsigned char) file_g;
			fractal->coarray[i].b = (unsigned char) file_b;
			printf("Setting index %d to %d,%d,%d.\n",i,file_r,file_g,file_b);
			i++;
		}
...
void fractal_init(flame *fractal) {
  ...
  fractal->num_threads = THREADGROUPSIZE;
	...
	if(fractal->choice == NULL) {
		printf("Error: malloc() failed in fractal_init.  Tried to malloc %d * %ld bytes.\n", fractal->count, sizeof(int));
		exit(EXIT_FAILURE);
	}
	fractal->choice[0] = 0;
}
...
void parse_args(int argc, char **argv, flame *fractal) {
	...
	while(i < argc)
	{
		if(!strcmp(argv[i], "-h")){
			...
			fractal->seed = atoi(argv[i+1]); i+=2;}
		else if(!strcmp(argv[i], "-T")){
			fractal->num_threads = atoi(argv[i+1]); i+=2;}
		else if(!strcmp(argv[i], "-f")){
			fractal->file = argv[i+1]; i+=2;}
		else if(!strcmp(argv[i], "-x")){
			fractal->xres = atoi(argv[i+1]); i+=2;}
		else if(!strcmp(argv[i], "-y")){
			fractal->yres = atoi(argv[i+1]); i+=2;}
		else if(!strcmp(argv[i], "-m")){
			fractal->xmin = (double) atof(argv[i+1]); i+=2;}
		else if(!strcmp(argv[i], "-M")){
			fractal->xmax = (double) atof(argv[i+1]); i+=2;}
		else if(!strcmp(argv[i], "-l")){
			fractal->ymin = (double) atof(argv[i+1]); i+=2;}	
		else if(!strcmp(argv[i], "-L")){
			fractal->ymax = (double) atof(argv[i+1]); i+=2;}
		else if(!strcmp(argv[i], "-S")){
			fractal->symmetry = atoi(argv[i+1]); i+=2;
			if(fractal->symmetry <= 0) fractal->symmetry=1;}
		else if(!strcmp(argv[i], "-n")){
			fractal->n = atoi(argv[i+1]); i+=2;}	
		else if(!strcmp(argv[i], "-s")){
			fractal->samples = atoi(argv[i+1]); i+=2;}		
		else if(!strcmp(argv[i], "-i")){
			fractal->iterations = atol(argv[i+1]) < 20 ? 1000 : atol(argv[i+1]); i+=2;}
		else if(!strcmp(argv[i], "-p")){
			if(((fractal->pallete=fopen(argv[i+1], "r")) == NULL)){
				printf("Error reading input file %s.\n",argv[i+1]);exit(EXIT_FAILURE);}i+=2;}
		else if(!strcmp(argv[i], "-c")){
			if(((fractal->cofile=fopen(argv[i+1], "r")) == NULL)){
				printf("Error reading input file %s.\n",argv[i+1]);exit(EXIT_FAILURE);}i+=2;}
		else if(!strcmp(argv[i], "-r")){
			fractal->R = atoi(argv[i+1]) % 256; i+=2;}
		else if(!strcmp(argv[i], "-g")){
			fractal->G = atoi(argv[i+1]) % 256; i+=2;}
		else if(!strcmp(argv[i], "-b")){
			fractal->B = atoi(argv[i+1]) % 256; i+=2;}
		else if(!strcmp(argv[i], "-G")){
			fractal->gamma = (((double) atof(argv[i+1])) == 0.0) ? 2.2 : (double) atof(argv[i+1]); i+=2;}
		else if(!strcmp(argv[i], "-sup")){
			fractal->sup = atoi(argv[i+1]) == 0 ? 1 : atoi(argv[i+1]); i+=2;}
		else if(!strcmp(argv[i], "-v"))
		{
			if(!override && fractal->count==1) { /* first user supplied param overrides default */
				...
				fractal->choice = realloc(fractal->choice, ((fractal->count+1) * sizeof(int)));
				if(fractal->choice == NULL) {
					printf("Error: malloc() failed in parse_args. Tried to allocate %d * %ld bytes.\n",fractal->count, sizeof(int));
					exit(EXIT_FAILURE);
				}
  ...
  int invert = fractal->invert;
	...
	for(row=0;row<(*fractal).yres;row++) {
		for(col=0;col<(*fractal).xres;col++) {
			...
			raster[(col*3)+1] = invert == 1 ? ~((*fractal).pixels[row][col].g): 
				(*fractal).pixels[row][col].g;
			raster[(col*3)+2] = invert == 1 ? ~((*fractal).pixels[row][col].b): 
  ...
  tran = 0;
	...
	for(num = 0; num < fractal->samples; num++)
	{
		...
		for(step= -20; step < fractal->iterations; step++)
		{
			...
			x = fractal->coarray[i].ac * newx + fractal->coarray[i].bc * newy + fractal->coarray[i].cc;
			y = fractal->coarray[i].dc * newx + fractal->coarray[i].ec * newy + fractal->coarray[i].fc;
			...
			switch(k)
			{
				...
				case 21: /* Rings */
					r = sqrt(x * x + y * y);
					theta = atan2(y, x);
					prefix = modulus((r + pa2 * pa2), (2.0 * pa2 * pa2)) - (pa2 * pa2) + (r * (1.0 - pa2 * pa2));
				...
				case 29: /* Mobius */
					...
					newx = ((pa1 * x + pa2) * (pa3 * x + pa4) + pa1 * pa3 * y * y) / t;
					newy = (pa1 * y * (pa3 * x + pa4) - pa3 * y * (pa1 * x + pa2)) / t;
					break;
				case 30: /* Blob */
					r = sqrt(x*x + y*y);
					theta = atan2(y,x);
			...
			if(step > 0)
			{
				...
				for(s=0;s<fractal->symmetry;s++) {
					...
					if(x_rot >= fractal->xmin && x_rot <= fractal->xmax && y_rot >= fractal->ymin && y_rot <= fractal->ymax)
					{
						x1 = fractal->xres - (unsigned int)(((fractal->xmax - x_rot) / fractal->ranx) * fractal->xres);
						y1 = fractal->yres - (unsigned int)(((fractal->ymax - y_rot) / fractal->rany) * fractal->yres);
          		...
          		pthread_mutex_lock(&(fractal->lock[y1]));
								...
								point->value.counter++;
								red = (unsigned char)((point->r + fractal->coarray[i].r) / 2);
								point->r = red;
								green = (unsigned char)((point->g + fractal->coarray[i].g) / 2);
								point->g = green;
								blue = (unsigned char)((point->b + fractal->coarray[i].b) / 2);
								point->b = blue;
...
void gamma_log(flame* fractal) {
	...
	for(col=0; col<fractal->xres; col++) {
		if(fractal->pixels[row][col].value.counter != 0) { 
				fractal->pixels[row][col].value.normal = (float) log10((double)fractal->pixels[row][col].value.counter);
	...
	for(row=0; row<fractal->yres; row++) {
		for(col=0; col<fractal->xres; col++) {
			...
			fractal->pixels[row][col].r = (unsigned char) ((float)(fractal->pixels[row][col].r)) * pow(fractal->pixels[row][col].value.normal, (1.0 / gamma));
			fractal->pixels[row][col].g = (unsigned char) ((float)(fractal->pixels[row][col].g)) * pow(fractal->pixels[row][col].value.normal, (1.0 / gamma));
			fractal->pixels[row][col].b = (unsigned char) ((float)(fractal->pixels[row][col].b)) * pow(fractal->pixels[row][col].value.normal, (1.0 / gamma));
		}
	}
	...
	for(y=0; y<(*fractal).yres; y++) {
		for(x=0; x<(*fractal).xres; x++) {
			...
			for(sy=0; sy<sample; sy++) {
				...
				count += (*fractal).pixels[y*sample+sy][x*sample+sx].value.counter;
				}
			}
...
int main(int argc, char **argv) {
  ...
  int i;
	...
	printf("Done!\n");
   /* correct for threads */
	if(fractal.num_threads <= 0) { fractal.num_threads = 1; }
  ...
  if(fractal.sup > 1) { reduce(&fractal); }
	/* write out the file */
	gamma_log(&fractal);
